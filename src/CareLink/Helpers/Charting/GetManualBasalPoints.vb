...
Friend Module GetManualBasalPoints
    ...
    Friend Function GetManualBasalValues(marker As IndexClass(Of Dictionary(Of String, String))) As SortedDictionary(Of OADate, Single)
        Dim timeOrderedMarkers As New SortedDictionary(Of OADate, Single)
        Dim markerEntry As Dictionary(Of String, String) = marker.Value
        If CurrentPdf.IsValid AndAlso Not markerEntry.GetBooleanValue(NameOf(LowGlucoseSuspendRecord.deliverySuspended)) Then
            Dim nextPumpSuspendTime As OADate
            If s_markers.Count > 1 AndAlso marker.Index = s_markers.Count - 2 Then
            ...
            Else
                nextPumpSuspendTime = New OADate(s_markers(marker.Index + 1).GetMarkerDateTime)
            End If
            ...
            Dim nextMarkerTime As New OADate(s_markers(marker.Index + 1).GetMarkerDateTime)
