...
Public Class Client2RefreshTests
    ...
    Public Async Function DoRefreshAsync_WithClientSecretInTokenData_Succeeds() As Task
        ...
        handler.ResponseFactory = Function(req)
                                      Return New HttpResponseMessage(statusCode:=HttpStatusCode.OK) With {.Content = New StringContent(content:=tokenResponseJson)}
                                  End Function
        ...
        Dim tokenJson As String = "{""access_token"":""aaa.bbb.ccc"",""refresh_token"":""r"",""client_id"":""cid"",""client_secret"":""secret""}"
        ...
        dict("access_token").GetString().Should().Be("new.access.token")
    End Function
    ...
    Public Async Function DoRefreshAsync_ResolverFallbackProvidesClientSecret_Succeeds() As Task
        ...
        Dim endpointResolver As Func(Of ConfigRecord, Task(Of EndpointConfig)) = Function(cfg)
        ...
        ' Act
        Dim result As JsonElement = Await client.DoRefreshAsync(client.Config, tokenElement, httpClient, endpointResolver)
        ...
        result.TryFromJson(result:=dict).Should().BeTrue()
        dict.Should().ContainKey("access_token")
        dict("access_token").GetString().Should().Be("fromresolver.token")
    End Function
    ...
    Public Async Function DoRefreshAsync_IncludesMagIdentifierHeader_WhenPresent() As Task
        ...
        Dim handler As New CapturingHandler(Function(req)
                                                capturedRequest = req
        ...
        Dim tokenJson As String = "{""access_token"":""old"",""refresh_token"":""r"",""client_id"":""cid"",""client_secret"":""secret"",""mag-identifier"":""mag123""}"
        Dim tokenElement As JsonElement = JsonSerializer.Deserialize(Of JsonElement)(json:=tokenJson)
        Dim bindingAttr As BindingFlags = BindingFlags.NonPublic Or BindingFlags.Instance
        Dim tokenField As FieldInfo = client.GetType().GetField(name:="_tokenDataElement", bindingAttr)
        tokenField.SetValue(obj:=client, value:=tokenElement)
        ...
        capturedRequest.Headers.Contains("mag-identifier").Should().BeTrue()
        capturedRequest.Headers.GetValues("mag-identifier").Should().Contain("mag123")
    End Function
