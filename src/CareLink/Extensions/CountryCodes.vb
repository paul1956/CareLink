...
Public Module RegionCountryLists
    ...
    Public Function ExtractCultureFromFileName(ReportFileNameWithPath As String, FixedPart As String, Optional fuzzy As Boolean = False) As CultureInfo
        ...
        If filenameWithoutExtension.Count("("c) <> 1 Then
            MsgBox($"Error Report Filename '{filenameWithoutExtension}' malformed,{s_environmentNewLine}it must contain exactly one '('.", MsgBoxStyle.OkOnly, "Malformed Error Report Filename")
            Return Nothing
        End If
        ...
        If Not filenameWithoutExtension.StartsWith(FixedPart) Then
            MsgBox($"Error Report Filename '{filenameWithoutExtension}' malformed,{s_environmentNewLine}it must start with '{FixedPart}'.", MsgBoxStyle.OkOnly, "Malformed Error Report Filename")
            Return Nothing
        End If
        ...
        If fuzzy Then
            If indexOfOpenParenthesis < FixedPart.Length Then
                MsgBox($"Error Report Filename '{filenameWithoutExtension}' malformed,{s_environmentNewLine}it must contain '(' after '{FixedPart}.", MsgBoxStyle.OkOnly, "Malformed Error Report Filename")
                Return Nothing
            End If
        Else
            If indexOfOpenParenthesis <> FixedPart.Length Then
                MsgBox($"Error Report Filename '{filenameWithoutExtension}' malformed,{s_environmentNewLine}it must contain '(' immediately after '{FixedPart}.", MsgBoxStyle.OkOnly, "Malformed Error Report Filename")
                Return Nothing
            End If
        End If
        ...
        If indexOfClosedParenthesis < 0 Then
            MsgBox($"Error Report Filename '{filenameWithoutExtension}' malformed,{s_environmentNewLine}it must contain ')'.", MsgBoxStyle.OkOnly, "Malformed Error Report Filename")
            Return Nothing
        End If
