...
Friend Module BrowserUtilities
    ...
    Private Function LaunchBrowser(url As String) As Boolean
        ...
        Using userChoiceKey As RegistryKey =
            Registry.CurrentUser.OpenSubKey(name)
            If userChoiceKey Is Nothing Then
                Return False
            End If
            Dim progIdObject As Object = userChoiceKey.GetValue(name:="ProgId")
            ...
            Dim programFiles As String =
                Environment.ExpandEnvironmentVariables(name:="%ProgramW6432%")
            Dim programFilesX86 As String =
                Environment.ExpandEnvironmentVariables(name:="%ProgramFiles(x86)%")
            Dim browserPath As String = Nothing
            Dim path As String
            If progIdValue.ContainsIgnoreCase(value:="chrome") Then
                path = $"{programFiles}\Google\Chrome\Application\chrome.exe"
                browserPath =
                    If(File.Exists(path),
                       path,
                       $"{programFilesX86}\Google\Chrome\Application\chrome.exe")
            ElseIf progIdValue.ContainsIgnoreCase(value:="Firefox") Then
                browserPath = $"{programFiles}\Mozilla Firefox\Firefox.exe"
            ElseIf progIdValue.ContainsIgnoreCase(value:="msEdgeHtm") Then
                path = $"{programFiles}\Microsoft\Edge\Application\msEdge.exe"
                browserPath = If(File.Exists(path),
                                 path,
